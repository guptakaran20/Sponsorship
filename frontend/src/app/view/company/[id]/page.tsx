...
import { Building2, Users, ArrowLeft, Target, Link as LinkIcon, Phone, User, Briefcase } from 'lucide-react';
...
export default function PublicCompanyProfilePage() {
    ...
    useEffect(() => {
        const fetchProfile = async () => {
            try {
                const data = await fetchApi(`/public/company/${params.id}`);
                setProfile(data);
            } catch (err: any) {
        ...
        <div className="min-h-screen bg-slate-950 p-4 lg:p-8">
            <div className="max-w-4xl mx-auto space-y-8">
                ...
                <div className="bg-slate-900 border border-white/5 rounded-3xl overflow-hidden shadow-xl">
                    <div className="h-32 bg-gradient-to-r from-amber-600/20 to-orange-600/20 px-8 flex items-end pb-6">
                        <div className="w-24 h-24 bg-amber-500 rounded-2xl flex items-center justify-center text-4xl font-bold text-white shadow-lg translate-y-12 border-4 border-slate-900">
                            {profile.userName?.charAt(0) || 'C'}
                        </div>
                    </div>
                            ...
                            <div>
                                ...
                                <div className="space-y-3">
                                    <div className="flex items-center text-slate-300">
                                        ...
                                        <span>Company Size: <strong className="text-white">{profile.companySize || 'Unknown'}</strong></span>
                                    </div>
                                    <div className="flex items-center text-slate-300">
                                        <Target className="w-4 h-4 mr-3 text-slate-500" />
                                        <span>Target Audience: <strong className="text-white">{profile.targetAudience || 'Any'}</strong></span>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div className="space-y-6">
                            {(profile.website || profile.socialLinks) && (
                                <div>
                                    ...
                                    <div className="space-y-3">
                                        {profile.website && (
                                            <a href={profile.website} target="_blank" rel="noreferrer" className="flex items-center text-amber-400 hover:text-amber-300 transition-colors">
